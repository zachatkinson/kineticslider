  ...
  return {
    ...
    stdDev: Math.sqrt(
      values.reduce(
        (sq, n) =>
          sq +
          Math.pow(n - values.reduce((a, b) => a + b, 0) / values.length, 2),
        0
      ) /
        (values.length - 1)
    ),
...
console.log(
  '| Metric | Value (avg) | P95 | Threshold | Status | Consistency |'
);
console.log(
  '|---------|------------|-----|-----------|---------|-------------|'
);
...
Object.entries(thresholds).forEach(([metric, threshold]) => {
  ...
  const formattedValue =
    metric === 'CLS'
      ...
      : metric.includes('Size')
        ...
        : metric.endsWith('time') ||
            ['FCP', 'LCP', 'TTI', 'FMP', 'Speed_Index'].includes(metric)
  ...
  console.log(
    `| ${metric} | ${formattedValue} | ${Math.round(stats.p95)} | ${formattedThreshold} | ${status} | ${consistency} |`
  );
});
...
if (failedMetrics.length > 0) {
  ...
  failedMetrics.forEach(([metric, threshold]) => {
    ...
    const avg = values.reduce((a, b) => a + b, 0) / values.length;
    console.log(
      `- ${metric}: ${Math.round(avg * 100) / 100} (Threshold: ${threshold})`
    );
  });
} else {
...
if (resourceMetrics.DOM_Size && resourceMetrics.Resource_Size) {
  ...
  console.log('|---------|--------|-----------|---------|');
  console.log(
    `| DOM Elements | ${Math.round(resourceMetrics.DOM_Size)} | ≤ ${thresholds.DOM_Size} | ${resourceMetrics.DOM_Size <= thresholds.DOM_Size ? '✅' : '❌'} |`
  );
  console.log(
    `| Total Resources | ${Math.round(resourceMetrics.Resource_Size / 1024)}KB | ≤ ${Math.round(thresholds.Resource_Size / 1024)}KB | ${resourceMetrics.Resource_Size <= thresholds.Resource_Size ? '✅' : '❌'} |`
  );
}
