...
import { MonitoringMetricCardProps, EventListProps } from '../src/types/migration';

const MetricCard: React.FC<MonitoringMetricCardProps> = ({ title, value, trend, status }) => (
...
export const MonitoringDashboard: React.FC = () => {
  const [metrics, setMetrics] = useState<Map<FeatureFlag, any>>(new Map());
  const [events, setEvents] = useState<any[]>([]);
  const [selectedFeature, setSelectedFeature] = useState<FeatureFlag | null>(null);

  useEffect(() => {
    const updateData = () => {
      setMetrics(featureMonitoring.getAllMetrics());
      setEvents(featureMonitoring.getRecentEvents(10));
    };
  ...
  const getFeatureHealth = (feature: FeatureFlag) => {
    const metrics = featureMonitoring.getFeatureMetrics(feature);
    if (!metrics) return 'unknown';
    
    const errorRate = featureMonitoring.getErrorRate(feature);
    if (errorRate > 0.1) return 'critical';
    if (errorRate > 0.05) return 'warning';
    if (metrics.usageCount === 0) return 'unused';
    return 'healthy';
  };
  ...
  return (
    <div className="p-6 bg-gray-50">
      <div className="mb-8">
        <h2 className="text-lg font-medium text-gray-900">Feature Monitoring</h2>
              ...
              <div className="bg-white overflow-hidden shadow rounded-lg">
                ...
                <div className="bg-gray-50 px-5 py-3">
                  <div className="text-sm">
                    ...
                    {metrics.lastUsed
                      ? new Date(metrics.lastUsed).toLocaleString()
                      : 'Never'}
                  </div>
                  {metrics.averagePerformance && (
                    <div className="text-sm">
                      ...
                      {metrics.averagePerformance.toFixed(2)}ms
                    </div>
                  )}
                </div>
              </div>
