...
function sliderReducer(state: SliderState, action: SliderAction): SliderState {
  switch (action.type) {
    case "NEXT":
      return {
        ...state,
        currentIndex: state.infiniteLoop
          ? createBrandedNumber(
    ...
    case "PREVIOUS":
      return {
        ...
        currentIndex: state.infiniteLoop
          ...
          : createBrandedNumber(
              Math.max((state.currentIndex as number) - 1, 0),
              "SlideIndex",
            ),
        isAnimating: true,
      };
  ...
  const goTo = useCallback(
    (index: number) => {
      if (!state.isAnimating && index !== state.currentIndex) {
        dispatch({
          type: "GO_TO",
          _index: createBrandedNumber(index, "SlideIndex"),
        });
      }
    },
...
export function withSlider<P extends object>(
  ...
  return function _WithSliderComponent(
    props: P & Omit<SliderProviderProps, "children">,
  ): unknown {
