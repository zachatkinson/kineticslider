...
export function hasMethod<K extends string>(
  ...
  return (
    isObject(value) &&
    methodName in value &&
    isFunction((value as Record<string, unknown>)[methodName])
  );
}
...
export function _isValidationResult(value: unknown): value is ValidationResult {
  ...
  return (
    "valid" in result && "errors" in result && Array.isArray(result.errors)
  );
}
 ...
 *
 */
export function _isPerformanceMetrics(
  value: unknown,
): value is PerformanceMetrics {
  if (!isObject(value)) return false;
  const metrics = value as Record<string, unknown>;
  return "fps" in metrics && "memoryUsage" in metrics;
}
...
export function _hasFocusFunction(value: unknown): value is Focusable {
  return (
    isObject(value) &&
    "focus" in value &&
    isFunction((value as Record<string, unknown>).focus)
  );
}
 ...
 *
 */
export function _hasInitialFocusFunction(
  value: unknown,
): value is InitialFocusable {
  return (
    isObject(value) &&
    "initialFocus" in value &&
    isFunction((value as Record<string, unknown>).initialFocus)
  );
}
