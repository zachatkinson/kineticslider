 ...
 */
export function memoize<T extends (...args: unknown[]) => unknown, R = ReturnType<T>>(
  func: T
): (...args: Parameters<T>) => R {
 ...
 */
export function getNestedValue<T, D = undefined, O extends Record<string, unknown> = Record<string, unknown>>(
  obj: O,
  path: string,
  defaultValue?: D
): T | D {
...
export function tryOperation<T>(operation: () => T): Result<T> {
  try {
    ...
    return createErrorResult<T>(error instanceof Error ? error : new Error(String(error)));
  }
}
