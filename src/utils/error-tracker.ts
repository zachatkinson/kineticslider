...
export class ErrorTracker {
  ...
  public captureError(
    ...
    const report: ErrorTrackerReport = {
      error:
        errorInstance instanceof SliderError
          ? errorInstance
          : new SliderError(
  ...
  public captureWarning(
    ...
    const report: ErrorTrackerReport = {
      error:
        warningInstance instanceof SliderError
          ? warningInstance
          : new SliderError(
  ...
  public addErrorListener(
    listener: (report: ErrorTrackerReport) => void,
  ): void {
  ...
  public getErrorsBySeverity(
    severity: ErrorTrackerContext["severity"],
  ): ErrorTrackerReport[] {
    return this.reports.filter(
      (report) => report.context.severity === severity,
    );
  }
  ...
  public getErrorsByComponent(component: string): ErrorTrackerReport[] {
    return this.reports.filter(
      (report) => report.context.component === component,
    );
  }
  ...
  private logReport(report: ErrorTrackerReport): void {
    ...
    console.warn(
      `[${timestamp}] ${context.severity.toUpperCase()}: ${error.message}`,
    );
