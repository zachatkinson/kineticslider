...
describe("usePerformance Hook", () => {
  beforeEach(() => {
    ...
    vi.spyOn(performanceUtils, "createPerformanceMonitor").mockReturnValue(
      () => {
        return;
      },
    );
    vi.spyOn(performanceUtils, "trackRenderTime").mockReturnValue(10);
    vi.spyOn(performanceUtils, "_createPerformanceComponentId").mockReturnValue(
      "test-component-id",
    );
  });
  ...
  it("should provide onMetricsUpdate callback to performance monitor", () => {
    ...
    const createMonitorSpy = vi.spyOn(
      performanceUtils,
