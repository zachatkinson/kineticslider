...
import { ValidationErrorType, ValidationErrorCode, ValidationResult } from '../../types/validation';
import {
  _isEmpty as isEmpty,
  ...
  describe('Validator Composition', () => {
    describe('composeValidators', () => {
      test('combines multiple validators', () => {
        const validator1 = (value: number): ValidationResult => ({
          valid: value > 0,
          errors: value > 0 ? [] : [createValidationError(
        ...
        const validator2 = (value: number): ValidationResult => ({
          valid: value < 100,
          errors: value < 100 ? [] : [createValidationError(
            ValidationErrorType.RANGE,
            ValidationErrorCode.OUT_OF_RANGE,
    ...
    describe('composeAsyncValidators', () => {
      test('combines multiple async validators', async () => {
        const asyncValidator1 = async (value: number): Promise<ValidationResult> => ({
          valid: value > 0,
          errors: value > 0 ? [] : [createValidationError(
            ValidationErrorType.RANGE,
            ValidationErrorCode.OUT_OF_RANGE,
        ...
        const asyncValidator2 = async (value: number): Promise<ValidationResult> => ({
          valid: value < 100,
          errors: value < 100 ? [] : [createValidationError(
            ValidationErrorType.RANGE,
            ValidationErrorCode.OUT_OF_RANGE,
        ...
        const composedValidator = composeAsyncValidators(asyncValidator1, asyncValidator2);
