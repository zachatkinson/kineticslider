...
export const AdvancedFilterManager: React.FC<AdvancedFilterManagerProps> = ({
  ...
  useEffect(() => {
    ...
    const available = allFilters.filter(name => !usedFilterNames.includes(name));
  ...
  const updateFilterSettings = useCallback((filterId: string, settings: Record<string, number | string | boolean>) => {
    setActiveFilters(prev => prev.map(f => 
      f.id === filterId ? { ...f, settings: { ...f.settings, ...settings } } : f
    ));
  ...
  const createSafeUpdate = (key: string, value: number | string | boolean): Record<string, number | string | boolean> => {
  ...
  function getDefaultSettings(filterName: string): Record<string, number | string | boolean> {
    // Common filter settings - could be enhanced with filter-specific defaults
    const commonSettings: Record<string, Record<string, number | string | boolean>> = {
      ...
      <div style={{ marginTop: '0.5rem', padding: '0.5rem', background: '#f9fafb', borderRadius: '4px' }}>
        {Object.entries(filter.settings).map(([key, value]) => (
          <div key={key} style={{ marginBottom: '0.5rem' }}>
            <label style={{ display: 'block', fontSize: '0.8rem', color: '#6b7280', marginBottom: '0.2rem' }}>
            ...
            {typeof value === 'number' ? (
              <input
                type="range"
                min="0"
                max={key === 'intensity' || key === 'alpha' ? "1" : key === 'size' ? "20" : "100"}
                step={key === 'intensity' || key === 'alpha' ? "0.1" : "1"}
                value={value}
                onChange={(e) => {
                  const updates = createSafeUpdate(key, parseFloat(e.target.value));
              ...
              <input
                ...
                style={{ width: '100%', padding: '0.2rem', borderRadius: '2px', border: '1px solid #d1d5db' }}
              />
            )}
            <span style={{ fontSize: '0.7rem', color: '#9ca3af' }}>{value}</span>
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div 
      ...
      <h3 style={{ margin: '0 0 1rem 0', color: '#374151' }}>🎨 Advanced Filter Manager</h3>
      ...
      <div style={{ marginBottom: '1rem' }}>
        {activeFilters.length === 0 ? (
          <p style={{ color: '#9ca3af', fontStyle: 'italic', margin: 0 }}>No filters active</p>
        ) : (
          activeFilters.map((filter) => (
            ...
            >
              ...
              <div style={{ opacity: filter.enabled ? 1 : 0.5, pointerEvents: filter.enabled ? 'auto' : 'none' }}>
      ...
      <div style={{ position: 'relative' }}>
        {availableFilters.length > 0 ? (
          ...
          <p style={{ color: '#9ca3af', fontStyle: 'italic' }}>All filters are in use</p>
        )}
      </div>
