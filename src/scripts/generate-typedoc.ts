...
import {
  ...
  TSConfigReader as _TSConfigReader,
  TypeDocReader as _TypeDocReader,
} from "typedoc";
...
const defaultConfig: TypeDocConfig = {
  ...
  options: {
    ...
    categoryOrder: ["Core", "Components", "Hooks", "Utilities", "Types", "*"],
  },
};

/**
 * Generate TypeDoc documentation for the project.
 *
 * @param config - The TypeDoc configuration.
 *
...
async function generateTypeDoc(config: TypeDocConfig): Promise<void> {
  ...
  try {
    ...
    if (project) {
      ...
      console.warn(
        `TypeDoc documentation generated successfully at ${config.outDir}`,
      );

      // Optionally generate JSON output for further processing
      await app.generateJson(
        project,
        path.join(config.outDir, "documentation.json"),
      );
      console.warn(
        `TypeDoc JSON data generated at ${path.join(config.outDir, "documentation.json")}`,
      );
    } else {
 ...
 *
 * @returns {TypeDocConfig} The merged TypeDoc configuration.
 *
 */
...
/**
 * Main entry point for the script. Parses configuration and generates TypeDoc documentation.
 *
